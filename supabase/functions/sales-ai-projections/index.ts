  ...
  try {
    ...
    switch (action) {
      case "generate_projection":
        systemPrompt = `Eres un analista financiero experto en restaurantes. Genera proyecciones de ventas basadas en datos históricos y factores externos.
Responde en JSON con:
- projected_revenue: número estimado
- confidence_level: porcentaje de confianza (0-100)
- factors: objeto con factores considerados
- ai_reasoning: explicación del análisis
...
Meta actual: $${data.current_goal || 0}`;
      ...
      case "analyze_goal_progress":
        systemPrompt = `Eres un coach de negocios para restaurantes. Analiza el progreso hacia la meta y proporciona insights accionables.
Responde en JSON con:
- status: "on_track", "at_risk", "behind"
- daily_target_needed: número
- probability_of_success: porcentaje
- action_items: array de acciones específicas
...
Progreso actual: ${data.progress_percent}%`;
      ...
      case "suggest_goals":
        systemPrompt = `Eres un estratega de restaurantes. Sugiere metas realistas y alcanzables basadas en el historial.
Responde en JSON con:
