...
export const TeamManagementTab: React.FC = () => {
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      switch (confirmAction.type) {
        case 'suspend':
          await suspendTeamMember(confirmAction.member.id);
          break;
        case 'reactivate':
          await reactivateTeamMember(confirmAction.member.id);
          break;
        case 'remove':
  ...
  const getInitials = (member: TeamMember) => {
    if (member.staff_member?.name) {
      return member.staff_member.name.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2);
    }
  ...
  const getMemberName = (member: TeamMember) => {
    return member.staff_member?.name || member.invited_email || 'Sin nombre';
  };

  const getMemberEmail = (member: TeamMember) => {
    return member.staff_member?.email || member.invited_email || '';
  };
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                          ...
                          {member.staff_member?.position && (
                            <>
                              <span>•</span>
                              <span>{member.staff_member.position}</span>
                            </>
                          )}
                        </div>
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            {member.status === 'invited' && (
                              <DropdownMenuItem
                                onClick={() => regenerateInvitation(member.id)}
          ...
          <DialogContent>
            ...
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">
                Comparte este link con el nuevo miembro del equipo. El link solo puede ser usado una vez.
              </p>
          ...
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
                ...
                Editar permisos - {getMemberName(editMember)}
              </DialogTitle>
            </DialogHeader>
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {confirmAction?.type === 'suspend' && (
                <>
                  El miembro <strong>{confirmAction && getMemberName(confirmAction.member)}</strong> no podrá
                  acceder al sistema hasta que se reactive su cuenta.
                </>
              )}
              {confirmAction?.type === 'reactivate' && (
                <>
                  El miembro <strong>{confirmAction && getMemberName(confirmAction.member)}</strong> volverá
                  a tener acceso al sistema con sus permisos anteriores.
                </>
              )}
              {confirmAction?.type === 'remove' && (
                <>
                  El miembro <strong>{confirmAction && getMemberName(confirmAction.member)}</strong> será
                  eliminado permanentemente del equipo. Esta acción no se puede deshacer.
                </>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
