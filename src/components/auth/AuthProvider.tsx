...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const handleSuccessfulLogin = async (session: Session, currentPath: string) => {
    logger.debug('🚀 handleSuccessfulLogin called:', {
    ...
    if (teamInviteToken) {
      logger.debug('🎟️ Found team invite token, claiming...');
      try {
        ...
        if (error) {
          logger.error('❌ Team invitation claim error:', error);
        } else if (result?.success) {
          logger.debug('✅ Team invitation claimed:', result);
          localStorage.removeItem('teamInviteToken');
          // Navigate to restaurant dashboard
          navigate('/r/dashboard', { replace: true });
          return;
        } else {
          logger.warn('⚠️ Team invitation claim failed:', result?.error);
        }
      } catch (err) {
        logger.error('💥 Team invitation claim exception:', err);
      }
      localStorage.removeItem('teamInviteToken');
    }
    ...
    if (!session?.user || isInProtectedRoute) {
      logger.debug('❌ Navigation skipped:', { currentPath, hasUser: !!session?.user, isInProtectedRoute });
      return;
    }
    ...
    if (fromPath && fromPath !== '/auth') {
      logger.debug('↩️ Returning to protected route:', fromPath);
    ...
    if (metaUserType === 'consultant') {
      logger.debug('🎯 Navigating to /c/dashboard (guard will redirect if needed)');
      navigate('/c/dashboard', { replace: true });
      return;
    }

    if (metaUserType === 'restaurant_owner') {
      logger.debug('🎯 Navigating to /r/dashboard (guard will redirect if needed)');
      navigate('/r/dashboard', { replace: true });
      return;
    }

    logger.debug('🎯 No user_type in metadata, navigating to /onboarding');
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      (event, session) => {
        ...
        if (event === 'SIGNED_IN' && session) {
          logger.debug('✅ SIGNED_IN event detected, deferring handleSuccessfulLogin');
          setTimeout(() => {
    ...
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      if (session) {
        try {
          ...
          if (isUserGone) {
            logger.warn('⚠️ Session belongs to a deleted user. Clearing.');
          ...
          logger.warn('⚠️ Could not validate session right now, keeping it.', e);
        }
      }
